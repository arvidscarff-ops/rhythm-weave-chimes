/**
 * SYS-010 LEGACY DEBUG ADAPTER.
 *
 * Preserves the original private-clock, listener, reset, and scrub-compatible
 * prototype for `/dev/transmissions` only. Product code must use the pure
 * supplied-time runtime in `transmissionRuntime.ts`.
 *
...
import { performanceTimeSource, type TimeSource } from "../crossing/timeSource";
import { stableUnitRoll } from "./rng";
import type { TransmissionDefinition } from "./transmissionTypes";

export type DebugTransmissionEndReason = "completed" | "arrival" | "reset";
export type DebugCrossingSnapshot = { crossingId: string; progress: number; phase: string };
export type DebugActiveTransmission = {
  definition: TransmissionDefinition;
  startedAtSeconds: number;
  endsAtSeconds: number;
};

export type DebugTransmissionRuntimeConfig = {
...
export type DebugTransmissionRunInfo = {
  crossingId: string;
  /** Explicit seed; falls back to the config seed, then to the crossing id. */
  seed?: string;
};

export type DebugTransmissionState = {
  crossingId: string | null;
  seed: string;
  current: DebugActiveTransmission | null;
...
export type DebugTransmissionListener = {
  transmissionStarted?: (t: DebugActiveTransmission, s: DebugTransmissionState) => void;
  transmissionEnded?: (
    t: DebugActiveTransmission,
    reason: DebugTransmissionEndReason,
    s: DebugTransmissionState,
  ) => void;
  eligibilityChanged?: (eligibleIds: string[], s: DebugTransmissionState) => void;
};
...
const inWindow = (d: TransmissionDefinition, p: number) => p >= d.windowStart && p < d.windowEnd;

const sameIds = (a: string[], b: string[]) =>
  a.length === b.length && a.every((v, i) => v === b[i]);

export function createTransmissionDebugRuntime(config: DebugTransmissionRuntimeConfig) {
  ...
  const listeners = new Set<DebugTransmissionListener>();

  let crossingId: string | null = null;
  let seed = config.seed ?? "";
  let current: DebugActiveTransmission | null = null;
  ...
  function snapshotState(): DebugTransmissionState {
    const now = time();
  ...
  function emit<K extends keyof DebugTransmissionListener>(
    key: K,
    call: (fn: NonNullable<DebugTransmissionListener[K]>) => void,
  ) {
    for (const l of Array.from(listeners)) {
      const fn = l[key];
      if (fn) call(fn as NonNullable<DebugTransmissionListener[K]>);
    }
  }

  function endCurrent(reason: DebugTransmissionEndReason, now: number) {
    if (!current) return;
    const ended = current;
    current = null;
   ...
   */
  function reconcile(snapshot?: DebugCrossingSnapshot): DebugTransmissionState {
    const now = time();
    ...
    const gapOk = lastEndedAtSeconds === null || now - lastEndedAtSeconds >= minGapSeconds;
    ...
    if (!arrived && !current && gapOk && crossingId !== null) {
      ...
      if (candidates.length > 0) {
        ...
        emit("transmissionStarted", (fn) =>
          fn(current as DebugActiveTransmission, snapshotState()),
        );
      }
    }
  ...
  return {
    ...
    startCrossing(run: DebugTransmissionRunInfo): DebugTransmissionState {
    ...
    update(snapshot: DebugCrossingSnapshot): DebugTransmissionState {
      return reconcile(snapshot);
    },

    /** Reconcile against time only, re-using the last observed snapshot. */
    sample(): DebugTransmissionState {
      return reconcile();
    },

    /** Read state without reconciling or emitting. */
    peek(): DebugTransmissionState {
      return snapshotState();
    },

    reset(): DebugTransmissionState {
      const active = current;
      const now = time();
      clearRunState();
      crossingId = null;
    ...
    subscribe(listener: DebugTransmissionListener): () => void {
...
export type TransmissionDebugRuntime = ReturnType<typeof createTransmissionDebugRuntime>;
