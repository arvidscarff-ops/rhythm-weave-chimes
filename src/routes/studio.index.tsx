...
function StudioOverview() {
  ...
  const r4Lab = studioR4Laboratory();
    ...
    <div className="space-y-8">
      <section className="grid gap-5 border border-white/10 bg-white/[0.025] p-6 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-medium tracking-tight">Private content inventory</h2>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-foreground/55">
            Packs, scales, scenes, presets, and the current comparative Trigger Engine laboratory
            evidence are inventoried here for review and backup.
          </p>
        </div>
      ...
      <section>
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="text-sm uppercase tracking-[0.18em] text-foreground/70">
            Comparative Trigger Engine laboratories
          </h3>
        </div>
        <div className="grid gap-3 lg:grid-cols-3">
          {r4Lab.families.map((family) => (
            <article key={family.prototypeId} className="border border-white/10 p-5">
              <div className="font-mono text-[9px] uppercase tracking-[0.18em] text-emerald-300/70">
                Shared exact authority · development-only
              </div>
              ...
              <dl className="mt-4 grid grid-cols-2 gap-3 font-mono text-[10px] uppercase tracking-[0.12em] text-foreground/45">
                <div>
                  <dt>Voices</dt>
                  <dd className="mt-1 text-foreground/80">{family.voiceIds.length}</dd>
                </div>
                <div>
                  <dt>Composition</dt>
                  <dd className="mt-1 truncate text-foreground/80">{r4Lab.composition.id}</dd>
                </div>
              </dl>
              <Button asChild variant="ghost" size="sm" className="mt-4 px-0">
                <Link to="/dev/trigger-engines">
                  Open comparative lab <ArrowUpRight className="ml-2 h-3.5 w-3.5" />
                </Link>
              </Button>
            </article>
          ))}
        ...
        <p className="mt-3 font-mono text-[9px] uppercase tracking-[0.14em] text-foreground/35">
          These geometries share one exact laboratory timeline. They are not selected production
          geometry, tuning, or public gateway direction.
        </p>
      </section>
